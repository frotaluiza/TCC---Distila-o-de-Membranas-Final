...
A destilação por membranas é um processo de separação térmico baseado no transporte de vapor através de uma membrana hidrofóbica e microporosa. A força motriz do processo é a diferença de pressão parcial de vapor entre os dois lados da membrana, geralmente estabelecida por uma diferença de temperatura entre as correntes. Nesse processo, a fase líquida é impedida de atravessar os poros da membrana, enquanto o vapor de água migra para o lado de menor pressão parcial, onde é posteriormente condensado (\cite{ExperimentalMD, S2MD}).
...
Adicionalmente, \cite{AndresManas2023} desenvolveram um modelo físico de estado estacionário baseado em balanços de massa e energia para um módulo V-AGMD em escala piloto, validado experimentalmente para predição de fluxo de permeado, temperaturas de saída e eficiência térmica.
...
Na destilação por membranas, modelos orientados por dados também têm sido amplamente empregados na previsão do desempenho do processo por meio de redes neurais artificiais. Essas redes são utilizadas para mapear diretamente variáveis operacionais em saídas de interesse, como fluxo de permeado e temperaturas de saída (\cite{ReviewAI_MD, ANN_VMD_Fouling}). Trabalhos como o de \cite{Requena2023} aplicaram redes neurais artificiais para modelar a qualidade do permeado em sistemas V-AGMD em escala piloto, utilizando um split simples 70/15/15 para treinamento, validação e teste.
...
Outra classe de abordagem envolve modelos residuais, nos quais um modelo físico fornece uma estimativa inicial e uma rede neural é treinada para aprender a diferença entre essa estimativa e os dados experimentais, combinando previsões baseadas em conhecimento físico com correções aprendidas a partir dos dados.
...
A Tabela~\ref{tab:principais_trabalhos_md} consolida os principais trabalhos revisados, organizados por tipo de abordagem, tecnologia de destilação por membranas e variáveis modeladas.

\begin{longtable}{p{3.2cm} p{1.8cm} p{1.8cm} p{3.8cm} p{3.8cm}}
\caption[Principais trabalhos de modelagem em MD revisados]{Principais trabalhos de modelagem em sistemas de destilação por membranas revisados, organizados por abordagem, tecnologia, modelo utilizado e variáveis preditas.}
\label{tab:principais_trabalhos_md}\\
\toprule
...
Alsaadi (2013) & Física & AGMD & Modelo unidimensional distribuído (difusão molecular + Knudsen) & Fluxo de permeado \\
Ansari (2022) & Física & AGMD & Modelo bidimensional com CFD (continuidade, momento, energia) & Fluxo de permeado \\
Lisboa et al. (2024) & Física & V-AGMD & Modelo físico reduzido 0D (balanços globais) & Fluxo de permeado, temperaturas de saída \\
Andrés-Mañas et al. (2023) & Física & V-AGMD & Balanços de massa e energia em estado estacionário & Fluxo, temperaturas, GOR, SEC \\
\midrule

\multicolumn{5}{l}{\textbf{Modelos orientados por dados}} \\
Kim et al. (2022) & Data-driven & V-AGMD & Regressão linear múltipla, rede neural generalizada, perceptron multicamadas & Fluxo de permeado \\
Requena et al. (2023) & Data-driven & V-AGMD & Rede neural artificial MLP com Levenberg-Marquardt & Fator de rejeição de sais, taxa de vazamento \\
Khayet e Cojocaru (2012a) & Data-driven & AGMD & Rede neural feed-forward com retropropagação & Fluxo de permeado, rejeição de sais \\
Shirazian e Alibabaei (2017) & Data-driven & AGMD & Rede neural com otimização por enxame de partículas & Fluxo, GOR, temperatura de saída \\
Yang (2023) & Data-driven & AGMD & Regressão linear múltipla, RNA com retropropagação, RNA de função de base radial & Fluxo de permeado, GOR \\
Jawed et al. (2025) & Data-driven & AGMD fototérmico & Rede neural com algoritmo genético; árvore de decisão, floresta aleatória & Fluxo de permeado \\
Khalifa et al. (2017) & Otimização & AGMD & Modelo físico com otimização por colônia de formigas e PSO & Fluxo de permeado \\
\midrule

\multicolumn{5}{l}{\textbf{Abordagens híbridas}} \\
Andrés-Mañas et al. (2024) & Híbrido & VMD & Physics-Informed Neural Network (equações na função de perda) & Fluxo de permeado, GOR \\
Abdulrahim et al. (2026) & Híbrido & AGMD & CFD combinado com rede neural substituta (surrogate) & Fluxo de permeado, consumo térmico específico \\
\midrule
\textbf{Este trabalho} & \textbf{Híbrido} & \textbf{V-AGMD} & \textbf{Regressões lineares, árvores de decisão, redes neurais e 4 arquiteturas híbridas} & \textbf{Fluxo de permeado, temperaturas de saída da alimentação e do resfriamento} \\
\end{longtable}
...
Alguns trabalhos, contudo, adotam abordagens mais robustas. O estudo de \cite{PINN_MD} utiliza uma abordagem baseada em PINNs para modelagem de processos de MD, enquanto \cite{ReviewAI_MD} apresenta uma rede neural com validação externa independente.
